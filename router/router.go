...
func Configure(m *wserver.Manager, client remote.Client) *gin.Engine {
	...
	protected.GET("/api/system/backups", getSystemBackupDestinations)
	...
	server.Use(middleware.RequireAuthorization(), middleware.ServerExists())
	{
		...
		server.POST("/reconcile", postServerReconcile)
		...
		files := server.Group("/files")
		{
			...
			files.GET("/trash", getServerTrash)
			files.POST("/trash/restore", postServerTrashRestore)
			files.POST("/trash/delete", postServerTrashDelete)
