...
type Client interface {
	...
	SetImportStatus(ctx context.Context, uuid string, data ImportStatusRequest) error
...
func (c *client) requestOnce(ctx context.Context, method, path string, body io.Reader, opts ...func(r *http.Request)) (*Response, error) {
	...
	req.Header.Set("User-Agent", fmt.Sprintf("FeatherPanel Wings/v%s (id:%s)", system.Version, c.tokenId))
