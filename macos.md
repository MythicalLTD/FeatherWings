...
- [mkcert](https://github.com/FiloSottile/mkcert) for SSL certificates
  (`brew install mkcert`)
- A featherpanel Panel instance with a node configured for this machine
...
```bash
mkdir -p ~/.config/featherpanel
mkdir -p ~/.local/share/featherpanel/{logs,volumes,archives,backups}
mkdir -p ~/.featherpanel/tmp
```
...
```bash
mkcert -install
mkcert -cert-file ~/.config/featherpanel/localhost.pem \
       -key-file ~/.config/featherpanel/localhost-key.pem \
       localhost 127.0.0.1
```
...
`/etc/featherpanel/config.yml` (or use `wings configure`) and save it to
`~/.config/featherpanel/config.yml`. Modify the following settings:

```yaml
api:
  ssl:
    enabled: true
    cert: /Users/<you>/.config/featherpanel/localhost.pem
    key: /Users/<you>/.config/featherpanel/localhost-key.pem
system:
  root_directory: /Users/<you>/.local/share/featherpanel
  log_directory: /Users/<you>/.local/share/featherpanel/logs
  data: /Users/<you>/.local/share/featherpanel/volumes
  archive_directory: /Users/<you>/.local/share/featherpanel/archives
  backup_directory: /Users/<you>/.local/share/featherpanel/backups
  tmp_directory: /Users/<you>/.featherpanel/tmp
  user:
    uid: 501    # your UID (run `id -u`)
    gid: 20     # your GID (run `id -g`)
    passwd:
      enable: true
      directory: /Users/<you>/.config/featherpanel
  machine_id:
    enable: false
...
- **All paths under `/Users/`** — Docker Desktop's file sharing only grants the
  ...
  default. Paths like `/var/lib/featherpanel` will not be accessible from inside
  containers.
...
- **`check_permissions_on_boot: false`** — Prevents Wings from trying to `chown`
  server data directories to a featherpanel system user.
...
```bash
./wings --config ~/.config/featherpanel/config.yml
```
...
If the Panel shows "is not featherpanel Wings!" after startup, clear the Panel cache:
